...
NOTE: Pour en savoir plus sur les technologies de réplication ONTAP, consultez https://["Documentation ONTAP"^].



=== Réplication des machines virtuelles de stockage (SVM) SnapMirror

Vous pouvez utiliser https://["SnapMirror"^] Pour répliquer un SVM complet, qui inclut ses paramètres de configuration et ses volumes. En cas d'incident, vous pouvez activer la SVM de destination SnapMirror pour démarrer le service des données. Vous pouvez revenir au primaire lorsque les systèmes sont restaurés.
