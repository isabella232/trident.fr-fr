...
----
. Utiliser `helm install` et spécifiez un nom pour votre déploiement comme dans l'exemple suivant où `23.07.0` Est la version d'Astra Trident que vous installez.
+
[listing]
----
...
|===
Par exemple, pour modifier la valeur par défaut de `debug`, exécutez ce qui suit `--set` commande où `23.07.0` Est la version d'Astra Trident que vous installez :
