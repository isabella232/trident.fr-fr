...
| `smbShare` | Vous pouvez spécifier le nom du partage SMB créé à l'aide de la console de gestion Microsoft du dossier partagé ou spécifier un nom de partage pour permettre à Astra Trident de créer le partage SMB. Sinon, vous pouvez laisser le paramètre vide pour empêcher l'accès commun au partage aux volumes. | `smb-share` 
...
.Exemple de configuration minimale pour ONTAP NAS Economy
[%collapsible%open]
====
Il s'agit d'une configuration de base utilisant le `ontap-nas-economy` conducteur.
...
----
====
.Exemple de configuration minimale de ONTAP NAS FlexGroup
[%collapsible%open]
====
Il s'agit d'une configuration de base utilisant le `ontap-nas-flexgroup` conducteur.
...
[%collapsible%open]
====
Dans cet exemple, Astra Trident doit utiliser des règles d'export dynamiques pour créer et gérer automatiquement les règles d'export. Cela fonctionne de la même manière pour le `ontap-nas-economy` et `ontap-nas-flexgroup` pilotes.
...
----
====
.Exemple d'utilisation d'adresses IPv6
[%collapsible%open]
====
...
[%collapsible%open]
====
Cet exemple crée un back-end Amazon FSX pour ONTAP à l'aide de volumes SMB.
